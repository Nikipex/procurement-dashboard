...
stock_live AS (
    SELECT
        ...
        GREATEST(COALESCE(t.total_stock_qty, 0) - COALESCE(r.reserved_stock_qty, 0), 0) AS free_stock_qty,
        CASE
...
SELECT *
FROM stock_live
WHERE total_stock_qty <> 0 OR reserved_stock_qty <> 0
ORDER BY free_stock_qty DESC, product_name
"""
...
out = "data/exports/live_procurement_stock_snapshot.csv"
